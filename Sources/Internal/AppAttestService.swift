    ...
    private func createKeyIdOperation(
        ...
        return AsyncClosureOperation<AppAttestKeyId>(operationClosure: { completion in
            service.generateKey { newKeyId, error in
                let result: Result<AppAttestKeyId, Error>
                defer {
                    completion(result)
                }

                guard let newKeyId else {
                    result = .failure(AppAttestServiceError.keyIdGeneration(error))
                    return
                }

                result = .success(newKeyId)
            }
        })
    }
        ...
        AsyncClosureOperation { completion in
            ...
            service.attestKey(keyId, clientDataHash: hash) { attestation, error in
                ...
                guard let attestation else {
                    ...
                    result = .failure(attestationError)
                    return
                }
                result = .success(.init(keyId: keyId, challenge: challenge, result: attestation))
            }
        }
    }
}
